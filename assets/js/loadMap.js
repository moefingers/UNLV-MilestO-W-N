...
*/
    ...
    if (heightValue / map.length < widthValue / map[0].length) {
        //height situation
    ...
    } else if (heightValue / map.length >= widthValue / map[0].length) {
        // width situation
    ...
    let controlOffset = fullControlSize * 0.02;
    ...
    let halfEdgeStartStyleGeneric = `width:${fullEdgeSize.length / 2}px;height:${
        fullEdgeSize.girth
    }px`;
    let halfEdgeEndStyleGeneric = `width:${fullEdgeSize.length / 2}px;height:${
        fullEdgeSize.girth
    }px`;
    ...
    for (let selectedRow = 0; selectedRow < map.length; selectedRow++) {
        //initialize row number as 0, select rows while less than map.length, etc.
        let currentRowArray = map[selectedRow]; //select row from map (for readability)
        for (let selectedBlock = 0; selectedBlock < currentRowArray.length; selectedBlock++) {
            // intialize block number as 0, select blocks while less than map.length, etc.
            ...
            if (selectedBlock + 1 >= currentRowArray.length) {
                rightControlProperties.active = 0;
            } else if (selectedBlock < currentRowArray.length) {
                rightControlProperties.active = currentRowArray[selectedBlock + 1];
            }
            ...
            if (selectedRow + 1 >= map.length) {
                belowControlProperties.active = 0;
            } else if (selectedRow < map.length) {
                belowControlProperties.active = map[selectedRow + 1][selectedBlock];
            }
            //console.log(`${belowControlProperties.pseudoCoordinates} active: ${belowControlProperties.active}`)

            if (controlProperties.active == true) {
                // this value will actually be 1, but using boolean for readability
                ...
                function horizontalEdge(controlCoordinates, locY) {
                    ...
                    controlHorizontal.style.translate = `${locationX + controlOffset}px ${
                        locY - fullEdgeSize.girth / 2
                    }px`;
                ...
                function verticalEdge(controlCoordinates, locX) {
                    ...
                    controlVertical.style.translate = `${locX + fullEdgeSize.girth / 2}px ${
                        locationY + controlOffset
                    }px`;
                ...
                if (rightControlProperties.active == false) {
                    verticalEdge(
                        rightControlProperties.pseudoCoordinates,
                        locationX + fullControlSize
                    );
                }
                horizontalEdge(controlProperties.pseudoCoordinates, locationY);
                if (belowControlProperties.active == false) {
                    horizontalEdge(
                        belowControlProperties.pseudoCoordinates,
                        locationY + fullControlSize
                    );
                }
